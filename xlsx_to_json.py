    ...
    if re.match(r'^[A-Z]\.$', second_name):
        ...
        if travel_doc in second_name_full_map:
            ...
            if full_name.startswith(first_letter):
                ...
                # Log the TravelDoc where there's a second name issue
                print(f"Problem with SecondName for TravelDoc {travel_doc}: Could not find matching full name for initial '{second_name}'")
        else:
